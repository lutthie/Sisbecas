    ...
    <div class="container-fluid">
      ...
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-bookmark"></i> Asignación</div>
        <div class="card-body">
        <!-- INICIO Config -->
            <body>
	<form method="POST" action="asignacionPrintsql.php">
		<h2>¡Felicidades! Usted ha sido aprobado</h2>
        <?php
        include ("php/conexion.php");
        $con=conectar();
        
        $query1=mysqli_query($con,"SELECT id_soli, nombre FROM SOLICITUD");
        $query2=mysqli_query($con,"SELECT id_beca, nombre FROM BECA");
        $query3=mysqli_query($con,"SELECT id_seguro, nombre_seguro FROM SEGURO");
        $query4=mysqli_query($con,"SELECT id_fin, nombre_finanza FROM FINANZA");
		...
		<div class="large-group">
			...
			<div class="small-group">
				<label for="datea">Fecha asginado</label>
				<input id="datea" type="date" name="datea"/>
			</div>
			
			<div class="small-group">
				<label for="carne">Carnet</label>
				<input for="carne" type="numeric" name="carne"/>
			</div>
			
            <div class="small-group">
				<label for="nosoli">Solicitud</label>
				<select name="nosoli">
                    <?php while($datos = mysqli_fetch_array($query1)){ ?>
					<option value="<?php echo $datos['id_soli']?>"><?php echo $datos['nombre']?></option>
					<?php }?>
				</select>
			</div>
            
            <div class="small-group">
				<label for="solibec">Beca solicitada</label>
				<select name="solibec">
            ...
            <div class="small-group">
				<label for="segur">Seguro a escoger</label>
				<select name="segur">
                    <?php while($datos = mysqli_fetch_array($query3)){ ?>
					<option value="<?php echo $datos['id_seguro']?>"><?php echo $datos['nombre_seguro']?></option>
					<?php }?>
				</select>
			</div>
            
            <div class="small-group">
				<label for="fianz">Plan de finanza</label>
				<select name="fianz">
                    <?php while($datos = mysqli_fetch_array($query4)){ ?>
					<option value="<?php echo $datos['id_fin']?>"><?php echo $datos['nombre_finanza']?></option>
					<?php }?>
				</select>
			</div>
            ...
            <div class="small-group">
				...
				<select name="user">
					...
					<option value="<?php echo $datos['id_usuario']?>"><?php echo $datos['name']?></option>
					<?php }?>
				</select>
			</div>
            
				<input id="submit" class="btn" type="submit" name="submit" value="Imprimir"/>
